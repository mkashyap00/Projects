...
def main(input_file):
    ...
    model = ElectraForSequenceClassification.from_pretrained(model_name, num_labels=3)  # Assuming 3 labels for classification
    ...
    train_loader = DataLoader(train_dataset_final, batch_size=16, shuffle=True, num_workers=0)  # Reduced batch size and num_workers for testing
    validation_loader = DataLoader(train_dataset_final, batch_size=16, num_workers=2)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch + 1}/{num_epochs}", leave=False):
            ...
            with autocast():
                ...
                loss = outputs.loss if hasattr(outputs, 'loss') else torch.nn.functional.cross_entropy(outputs.logits, labels)
    ...
    val_preds = torch.argmax(val_outputs.logits, axis=1).cpu().numpy()

    cm = confusion_matrix(val_labels.argmax(axis=1), val_preds)
